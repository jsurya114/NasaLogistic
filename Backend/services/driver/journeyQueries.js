...
export const updateSeqRouteCodeToDeliveriesTable = async ()=>{
    ...
    console.log(`✅ Updated ${result.rowCount} seq_route_code entries`);
    return result;

    } catch (error) {
    console.error('❌ Error updating seq_route_code for deliveries table:', error.message);
    return { success: false, message: "Error updating seq_route_code", error: error.message };
  }
}
...
export const markNoAddressAsNoScanned = async ()=>{
  try {
      ...
      `
 const result = await pool.query(query);
    console.log(`✅ Marked ${result.rowCount} entries as no_scanned`);
    return result;
  } catch (error) {
    console.error('❌ Error marking no address as no scanned:', error.message);
    return { success: false, message: "Error updating deliveries status", error: error.message };
  }
}
